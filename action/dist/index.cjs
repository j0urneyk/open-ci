...
// src/runner-contract.ts
var OpenCiConfigError = class extends Error {
  name = "OpenCiConfigError";
};
var ProviderQueryError = class extends Error {
  name = "ProviderQueryError";
};
var OpenCiCancellationError = class extends Error {
  name = "OpenCiCancellationError";
};
...
function parseRunnerConfig(text, format) {
  if (Buffer.byteLength(text, "utf8") > MAX_CONFIG_BYTES) invalidConfig("document", "exceeds 48 KiB");
  if (text.trim() === "") return {};
  let value;
  try {
    if (format === "json") JSON.parse(text);
    const document = (0, import_yaml.parseDocument)(text, { schema: "core", uniqueKeys: true, merge: false });
...
function mergeRunnerObjects(base, override) {
  const merged = { ...base };
  for (const [key, value] of Object.entries(override)) {
    const previous = merged[key];
    merged[key] = key !== "runs-on" && isConfigObject(previous) && isConfigObject(value) ? mergeRunnerObjects(previous, value) : value;
  }
  return merged;
}
function assertRunnerLabel(value, path) {
...
function assertRunnerLabels(value, path) {
  if (Array.isArray(value)) {
...
function validateRunnerTarget(value) {
  if (isConfigObject(value)) {
    assertConfigKeys(value, ["group", "labels"], "runs-on");
    if (value.group === void 0 && value.labels === void 0) invalidConfig("runs-on", "requires group or labels");
    if (value.group !== void 0) assertRunnerLabel(value.group, "runs-on.group");
    if (value.labels !== void 0) assertRunnerLabels(value.labels, "runs-on.labels");
  } else assertRunnerLabels(value, "runs-on");
  return value;
}
function validateProviderPriority(value) {
  if (!Array.isArray(value) || value.length === 0) invalidConfig("priority", "requires a nonempty array");
  for (const entry of value) {
    if (!PROVIDER_NAMES.includes(entry)) invalidConfig("priority", "contains an unknown provider");
  }
  if (new Set(value).size !== value.length) invalidConfig("priority", "must not contain duplicates");
  return value;
}
function assertFiniteNumber(value, path, allowZero) {
...
function resolveRunnerConfig(base, override, priorityInput = "") {
  ...
  if (priorityInput.trim()) {
    try {
      value.priority = JSON.parse(priorityInput);
    } catch {
      invalidConfig("priority input", "must be a JSON array");
    }
  }
  ...
  for (const [name, raw] of Object.entries(value.providers)) {
    ...
    if (raw.enabled === void 0) raw.enabled = true;
    ...
    if (raw["reserve-percent"] === void 0) raw["reserve-percent"] = 5;
...
var defaultGitHubTransport = { fetch: globalThis.fetch, sleep: import_promises.setTimeout };
var MAX_GITHUB_RETRY_WAIT_MS = 5e3;
var githubCredentialCooldowns = /* @__PURE__ */ new WeakMap();
function githubRetryDeadline(response, now) {
  const retryAfter = response.headers.get("retry-after");
  const exhausted = response.headers.get("x-ratelimit-remaining") === "0";
  const rateLimited = response.status === 429 || response.status === 403;
  if (!rateLimited && retryAfter === null) return void 0;
  let deadline = now;
  if (retryAfter !== null) {
    const value = retryAfter.trim();
    if (/^\d+$/.test(value) && Number.isSafeInteger(Number(value))) deadline = now + Number(value) * 1e3;
    else {
  ...
  if (exhausted) {
    ...
    deadline = Math.max(deadline, Number(reset) * 1e3);
  } else if (rateLimited && retryAfter === null) deadline = now + 6e4;
  return deadline;
}
async function waitForGitHubRetry(token, transport, cooldowns, backoff) {
...
async function requestGitHubJson(path, token, options = {}, transport = defaultGitHubTransport) {
  ...
  if (!cooldowns) {
    ...
    githubCredentialCooldowns.set(transport, cooldowns);
  }
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (deadline !== void 0) cooldowns.set(token, Math.max(cooldowns.get(token) ?? 0, deadline));
      ...
      try {
        ...
        throw new ProviderQueryError("GitHub response invalid: expected JSON.");
      }
...
function requireGitHubObject(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new ProviderQueryError("GitHub response invalid: expected an object.");
  return value;
}

// src/caller-repository.ts
async function loadCallerRepository(repository, sha, token, transport = defaultGitHubTransport) {
...
async function loadRepositoryConfig(caller, configPath, explicitPath, token, transport = defaultGitHubTransport) {
  ...
  try {
    ...
    throw new OpenCiConfigError("Open CI config file invalid: policy is not UTF-8 text.");
  }
}

// src/billing-period.ts
function calculateBillingPeriod(cycle, now) {
...
// src/github-billing-auth.ts
var import_node_crypto = require("node:crypto");
async function acquireBillingToken(owner, credentials, transport = defaultGitHubTransport, now = /* @__PURE__ */ new Date()) {
  ...
  try {
    ...
    jwt = `${unsigned}.${(0, import_node_crypto.createSign)("RSA-SHA256").update(unsigned).sign(credentials.privateKey, "base64url")}`;
  } catch {
    throw new ProviderQueryError("GitHub billing authentication invalid: unable to sign the App JWT.");
  }
  ...
  return { token, release: async () => {
    await requestGitHubJson("/installation/token", token, { method: "DELETE" }, transport);
  } };
}

// src/github-billing-repositories.ts
async function* readGitHubBillingPages(firstPath, token, transport = defaultGitHubTransport) {
  ...
  while (path) {
    ...
    if (next) {
      ...
      try {
        ...
        throw new ProviderQueryError("GitHub usage invalid: malformed pagination URL.");
      }
...
async function listPrivateBillingRepositories(owner, year, month, token, transport = defaultGitHubTransport) {
  ...
  for await (const payload of readGitHubBillingPages(firstPath, token, transport)) {
    ...
    for (const value of report.usageItems) {
      ...
      if (item.organizationName !== void 0 && (typeof item.organizationName !== "string" || item.organizationName.toLowerCase() !== owner.toLowerCase())) throw new ProviderQueryError("GitHub usage invalid: billed repository organization mismatch.");
...
function evaluateUsageAllowance(usage, freeMinutes, reservePercent) {
  ...
  return usage.usedMinutes < limit ? { state: "available", reason: `Usage ${usage.usedMinutes} is below the ${limit} minute selection threshold.`, usage } : { state: "unavailable", reason: `Usage ${usage.usedMinutes} has reached the ${limit} minute selection threshold.`, usage };
}
async function selectRunnerProvider(config, evaluate) {
  const attempts = [];
...
// src/github-runner-usage.ts
var STANDARD_RUNNER_LABELS = /* @__PURE__ */ new Set([
  "ubuntu-slim",
  "ubuntu-latest",
  "ubuntu-22.04",
  "ubuntu-24.04",
...
function isStandardGitHubRunner(target) {
  const labels = typeof target === "object" && !Array.isArray(target) ? target.group === void 0 ? target.labels : void 0 : target;
  const list = typeof labels === "string" ? [labels] : labels;
  return list?.length === 1 && STANDARD_RUNNER_LABELS.has(list[0] ?? "");
}
function normalizeGitHubUsage(payload, owner, year, month, multipliers) {
  const report = requireGitHubObject(payload);
...
async function evaluateGitHubRunner(config, caller, credentials, now = /* @__PURE__ */ new Date(), transport = defaultGitHubTransport) {
...
// src/blacksmith-runner-usage.ts
...
function validateBlacksmithDirectory(directory) {
  const path = (0, import_node_path.resolve)(directory);
...
async function registerBlacksmithCleanup(directory, stateFile = process.env.GITHUB_STATE) {
  const path = validateBlacksmithDirectory(directory);
  if (stateFile) await (0, import_promises2.appendFile)(stateFile, `blacksmithDirectory=${path}
`);
}
async function cleanupBlacksmithDirectory(directory) {
  await (0, import_promises2.rm)(validateBlacksmithDirectory(directory), { recursive: true, force: true });
}

// src/blacksmith-runner-usage.ts
var BLACKSMITH_VERSION = "0.4.60";
var BLACKSMITH_LINUX_SHA256 = "5ace4f255ae26b59c230ab8b22ee584f726266d7c1abebda2a953c27224065c0";
async function installBlacksmithCli(directory, fetchBinary = globalThis.fetch, sleep = import_promises4.setTimeout) {
  const executable = (0, import_node_path2.join)(directory, "blacksmith");
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await fetchBinary(`https://clireleases.blacksmith.sh/cli/v${BLACKSMITH_VERSION}/linux/amd64/blacksmith`, { signal: AbortSignal.timeout(3e4), redirect: "error" });
      if (!response.ok) {
        await response.body?.cancel();
        throw new Error("download status");
      }
      const buffer = Buffer.from(await response.arrayBuffer());
      if (buffer.length > 100 * 1024 * 1024 || (0, import_node_crypto2.createHash)("sha256").update(buffer).digest("hex") !== BLACKSMITH_LINUX_SHA256) throw new ProviderQueryError("Blacksmith CLI invalid: pinned binary checksum mismatch.");
      await (0, import_promises3.writeFile)(executable, buffer, { mode: 448 });
      await (0, import_promises3.chmod)(executable, 448);
      return executable;
    } catch (error) {
      if (error instanceof ProviderQueryError) throw error;
...
async function runBlacksmithCommand(executable, args, credentialDirectory, stdin) {
  return await new Promise((resolve2, reject) => {
    ...
    child.on("error", () => {
      ...
      reject(new ProviderQueryError("Blacksmith CLI unavailable: process could not start."));
    });
    child.on("close", (code) => {
      clearTimeout(timer);
      removeCancellationHandlers();
...
function blacksmithObject(value) {
...
function normalizeBlacksmithUsage(payload, owner, start, end) {
...
async function evaluateBlacksmithRunner(config, owner, token, now = /* @__PURE__ */ new Date(), transport = { install: installBlacksmithCli, sleep: import_promises4.setTimeout, platform: process.platform, arch: process.arch, run: runBlacksmithCommand }) {
  ...
  const queryTime = new Date(Math.floor(now.getTime() / 1e3) * 1e3);
  const period = calculateBillingPeriod(config["billing-cycle"], queryTime);
  const directory = await (0, import_promises3.mkdtemp)((0, import_node_path2.join)(process.env.RUNNER_TEMP || (0, import_node_os2.tmpdir)(), "open-ci-blacksmith-"));
  try {
    ...
    try {
      ...
      throw new ProviderQueryError("Blacksmith usage invalid: CLI output is not JSON.");
    }
...
async function writeActionOutput(name, value) {
  ...
  const delimiter = `open_ci_${(0, import_node_crypto3.randomUUID)()}`;
  await (0, import_promises5.appendFile)(file, `${name}<<${delimiter}
${value}
${delimiter}
`);
}
async function executeRunnerSelection() {
  ...
  try {
    ...
    if (process.env.GITHUB_STEP_SUMMARY) await (0, import_promises5.appendFile)(process.env.GITHUB_STEP_SUMMARY, renderSelectionSummary(result.attempts, config, source));
    ...
    if (error instanceof NoRunnerAvailableError && process.env.GITHUB_STEP_SUMMARY) await (0, import_promises5.appendFile)(process.env.GITHUB_STEP_SUMMARY, renderSelectionSummary(error.attempts, config, source));
    throw error;
  }
}
executeRunnerSelection().catch((error) => {
  const message = error instanceof OpenCiConfigError || error instanceof ProviderQueryError || error instanceof NoRunnerAvailableError || error instanceof OpenCiCancellationError ? error.message : "Open CI execution failed: unexpected runtime error. Check the runner prerequisites.";
