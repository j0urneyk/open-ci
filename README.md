...
To share one selection across several workflows, pass its JSON runner target to each reusable workflow. The [shared-selection example](examples/shared-selection/README.md) includes a parent and two children, copy instructions, and failure handling. The selector finishes before the children start; their individual checks and logs remain part of the parent run.
